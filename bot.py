...
async def geocode(address: str):
    async with aiosqlite.connect(DB) as db:
        ...
        row = await cur.fetchone()
        if row and row[0] >= GEOCODE_HARD_CAP: return None
    params = {"address": address, "components": "country:IT", "language": "it", "key": os.getenv("GOOGLE_API_KEY")}
    ...
    if "street_number" not in comps or best["geometry"]["location_type"] != "ROOFTOP": return None
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        await db.execute(
            "INSERT INTO geostats(month,cnt)VALUES(?,1) ON CONFLICT(month)DO UPDATE SET cnt=cnt+1", (month,))
        await db.commit()
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, timeout=10) as r:
                if r.status == 200:
                    return (await r.json()).get("address")
...
def fmt(st, p, avg, fuel):
    ...
    lines = [
        ...
        f"<b>Risparmio:</b> {perc}% rispetto alla media ({avg:.3f} € al L)",
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "service" and txt in SERVICE_MAP:
        ...
        ctx.user_data.clear();
        return
    await update.message.reply_text("Vuoi trovare il distributore più economico?\nDigita /trova oppure /start.")


async def profilo(update: Update, _):
    u = await get_user(update.effective_user.id)
    msg = "Nessun profilo. Usa /start." if not u else f"Fuel: {u[0]}\nServizio: {u[1]}"
...
async def handle_address(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    geo = await geocode(update.message.text)
    if not geo:
        await update.message.reply_text("Indirizzo troppo generico o non valido. Aggiungi civico o invia la posizione.")
        return LOC_STATE
    await process_coords(update, ctx, *geo);
    return ConversationHandler.END


async def process_coords(update: Update, ctx: ContextTypes.DEFAULT_TYPE, lat: float, lng: float):
    prefs = await get_user(update.effective_user.id);
    fuel, service = prefs
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        for (uid,) in await cur.fetchall():
            ...
            msg = (f"Grazie a TrovaBenzinaBot, lo scorso mese hai risparmiato: {saving:.2f}€*.\n\n"
                   ...
                   "*Calcolo su 50 € di rifornimento ogni ricerca.")
            try:
